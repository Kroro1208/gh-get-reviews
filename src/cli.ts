...
`;
    ...
    try {
      // Write config file with restrictive permissions (0600) to reduce accidental exposure
      fs.writeFileSync(homeEnvPath, envTemplate, {
        encoding: "utf8",
        mode: 0o600,
      });
      ...
      console.log("");
      console.log(
        "⚠️ セキュリティ注意: このファイルはプレーンテキストです。トークンをファイルに保存することは推奨されません。"
      );
      console.log(
        "   可能であれば環境変数 (GITHUB_TOKEN) または CI の Secrets (例: GitHub Actions Secrets) を使用してください。"
      );
      console.log("");
      ...
      console.log(
        '2. Edit the config file and replace "your_token_here" with your actual token (if you choose to store it)'
      );
      console.log(
        "3. Ensure the file permissions are kept private (it is created with 0600)"
      );
      console.log("4. Run your command again!");
      console.log("");
      console.log("💡 Edit config safely: nano ~/.get-gh-reviews.env");
      console.log("");
    } catch (error) {
...
program
  ...
  .action(async (options: ReviewsOptions) => {
    ...
    if (!validateGitHubUsername(options.username)) {
      console.error(
        "❌ 無効なGitHubユーザー名です。英数字とハイフンのみ使用可能です（最大39文字）。"
      );
      process.exit(1);
    }
    ...
    if (!token) {
      ...
      console.error("");
      console.error(
        "💡 Or use --token option: get-gh-reviews reviews -u username -t your_token"
      );
      process.exit(1);
    }
    ...
    try {
      ...
      if (options.markdown) {
        ...
        if (!fullPath) {
          console.error(
            "❌ 無効なファイル名です。英数字、ハイフン、アンダースコアのみ使用可能です。"
          );
          process.exit(1);
        }
        ...
        result.reviews.forEach((review, index) => {
          ...
          console.log(
            `${index + 1}. ${stateEmoji[review.state] || "❓"} ${review.state}`
          );
          ...
          if (review.body && review.body.trim()) {
            console.log(
              `   Comment: ${review.body.substring(0, 100)}${review.body.length > 100 ? "..." : ""}`
            );
          }
          console.log("");
        });
  ...
  .action(async (options: StatsOptions) => {
    ...
    try {
      ...
      if (options.json) {
        ...
        console.log(
          `   🔄 Changes Requested: ${stats.by_state.changes_requested}`
        );
